...
| table | { <code><br/>props: ITableProps;<br/>changeProps: React.Dispatch;<br/>onDispatch: OnDispatchFunc;<br/>dispatch: DispatchFunc; </code> <br/>[<code> ...actionCreators</code>](/docs_action.html#predefined-actions) <br/>}| object is used only by *uncontrolled mode* to get/set table props, use `useTable()` to create your own instance and handle changes using [onDispatch](https://github.com/komarovalexander/ka-table/blob/master/src/lib/types.ts); </code> <br/>[<code> ...actionCreators</code>](/docs_action.html#predefined-actions), and use `useTableInstance()` to get instance of table from context |
